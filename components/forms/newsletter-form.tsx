...
export function NewsletterForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid gap-3 sm:grid-cols-[1fr_auto]">
      <div>
        <Input type="email" placeholder="work@email.com" aria-label="Email address" {...register("email")} />
        {errors.email ? <p className="mt-2 text-sm text-red-600">{errors.email.message}</p> : null}
        {isSubmitSuccessful ? (
          <p className="mt-2 text-sm text-green-600">Thanks. You are on the update list.</p>
        ) : null}
      </div>
